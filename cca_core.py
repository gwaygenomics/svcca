...
to deep neural networks. The main function is get_cca_similarity, which takes
in two sets of activations, typically the neurons in two layers and their
outputs on all of the datapoints D = [d_1,...,d_m] that have been passed
through.

Inputs have shape (num_neurons1, m), (num_neurons2, m). This can be directly
applied used on fully connected networks. For convolutional layers, the 3d
block of neurons can either be flattened entirely, along channels, or
alternatively, the dft_ccas (Discrete Fourier Transform) module can be used.
...
def compute_ccas(sigma_xx, sigma_xy, sigma_yx, sigma_yy, verbose=True):
    ...
    preprocesses them (removing small magnitudes) and outputs the raw results
    of the cca computation, including cca directions in a rotated space, and
    the cca correlation coefficient values.
    ...
    Returns:
            ...
            invsqrt_xx:   Inverse square root of sigma_xx to transform
                          canonical directions back to original space
...
def get_cca_similarity(acts1, acts2, threshold=0.98, compute_dirns=True,
    ...
    Returns:
            return_dict: A dictionary with outputs from the cca computations.
                         ...
                         that correspond to cca directions), the cca
...
def robust_cca_similarity(acts1, acts2, threshold=0.98, compute_dirns=True,
    ...
    for trial in range(num_cca_trials):
        try:
            return_dict = get_cca_similarity(acts1, acts2, threshold,
                                             compute_dirns, verbose=verbose)
